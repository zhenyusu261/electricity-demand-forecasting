...
def make_rolling_input(values, scaler_X):
    """使用最近 SEQ_LENGTH 个观测构造单步预测输入，不重新拟合 scaler。"""
    values = np.asarray(values, dtype="float64")
...
def tune_component(name, X_train, y_train, X_val, y_val, settings):
    """对趋势项或周期项执行一次 Hyperband 搜索，并保存最优模型。"""
    tuner = kt.Hyperband(
        build_cnn,
        objective="val_loss",
    ...
    with open(MODEL_DIR / f"{name}_hyperparameters.json", "w", encoding="utf-8") as f:
...
# 训练窗口内的 X13 与 HP 分解
result_train, _, cycle_train, trend_train = run_x13(
    train_elec, train_exog
)
...
result_path = OUTPUT_DIR / f"X13-HP-CNN-ATT_{FORECAST_YEAR}_results.csv"
