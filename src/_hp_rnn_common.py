...
RECURRENT_CLASS = layers.LSTM
HP_UNIT_NAMES = ["units1", "units2"]


def build_rnn(hp):
    input_layer = layers.Input(shape=(SEQ_LENGTH, 1), name="sequence")
    layer1 = RECURRENT_CLASS(
        units=hp.Int(HP_UNIT_NAMES[0], 24, 256, step=24),
        return_sequences=True,
    )
    layer2 = RECURRENT_CLASS(
        units=hp.Int(HP_UNIT_NAMES[1], 24, 256, step=24),
        return_sequences=True,
    )
    hidden1 = layer1(input_layer)
    hidden2 = layer2(hidden1)

    attention = layers.Attention(use_scale=True)([hidden2, hidden2])
...
def run_variant(variant_name, recurrent_class, hp_names):
    global RECURRENT_CLASS, HP_UNIT_NAMES
    RECURRENT_CLASS = recurrent_class
    HP_UNIT_NAMES = hp_names
    ...
    result_path = OUTPUT_DIR / f"{variant_name}_{FORECAST_YEAR}_results.csv"
