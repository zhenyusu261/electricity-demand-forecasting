...
def _open_and_read(fname):
    ...
    with open(fname, encoding="cp1252") as fin:
        fout = fin.read()
    return fout
...
def x13_arima_analysis(endog, maxorder=(2, 1), maxdiff=(2, 1), diff=None,
    ...
    spec += "estimate{ tol = 0.1e-3 } \n"
    spec += "x11{ save=(d10 d11 d12 d13 d1) }"
    if speconly:
        return spec
    ...
    seasonfactor = _convert_out_to_series(seasonfactor, endog.index, 'seasonfactor')
    ...
    d1 = _convert_out_to_series(d1, endog.index, 'd1')
    ...
    if not retspec:
        res = X13ArimaAnalysisResult(observed=endog, results=results,
                                     seasonfactor =seasonfactor, seasadj=seasadj, trend=trend,
                                     irregular=irregular, d1=d1, stdout=stdout)
    else:
        res = X13ArimaAnalysisResult(observed=endog, results=results,
                                     seasonfactor =seasonfactor, seasadj=seasadj, trend=trend,
                                     irregular=irregular, d1=d1, stdout=stdout,
