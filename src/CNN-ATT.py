...
# CNN-ATT 直接对原始序列建模，不使用 X13，也不进行 HP 分解。
# 调参结构、数据划分、标准化与滚动一步预测方式与 X13-HP-CNN-ATT 保持一致。
FAST_MODE = False
...
def tune_cnn(X_train, y_train, X_val, y_val):
    """对 CNN-ATT 执行 Hyperband 搜索，并保存最优模型。"""
    tuner = kt.Hyperband(
        build_cnn,
        objective="val_loss",
        ...
        overwrite=True,
        directory=str(TUNER_DIR / "cnn_att"),
        project_name="cnn_att_model",
    )
    ...
    best_model.save(str(MODEL_DIR / "cnn_att_model.keras"))
    with open(
        MODEL_DIR / "cnn_att_hyperparameters.json",
        "w",
        encoding="utf-8",
    ) as f:
        json.dump(best_hps, f, ensure_ascii=False, indent=2)
    print(f"CNN-ATT best hyperparameters: {best_hps}")
    return best_model
...
for forecast_month in forecast_months:
    ...
    final_pred = predict_next(known_values.values)
...
result_path = OUTPUT_DIR / f"CNN-ATT_{FORECAST_YEAR}_results.csv"
